...
#define MBEST_MAX_STAGES 5         /* maximum number of VQ stages  */

struct MBEST_LIST {
    int   index[MBEST_MAX_STAGES]; /* index of each stage that lead us to this error */
    float error;
};

struct MBEST {
    int                entries;    /* number of entries in mbest list   */
    int                stages;     /* number of VQ stages               */
